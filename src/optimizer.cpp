...
DailyItinerary Optimizer::optimizeDay(NodeID hotel,
  ...
  for (size_t i = 0; i < path.size(); ++i) {
    ...
    if (i > 0) {
      if (auto r =
              m_router.findRoute(nodes[path[i - 1]], currentNode, metric)) {
        if (!di.fullPath.empty())
          di.fullPath.pop_back();
        di.fullPath.insert(di.fullPath.end(), r->path.begin(), r->path.end());
      } else
        di.fullPath.push_back(currentNode);
    } else
      di.fullPath.push_back(currentNode);
  }
  return di;
...
OptimizationResult Optimizer::optimize(NodeID hotel,
  ...
  for (auto &c : clusters)
    res.days.push_back(optimizeDay(hotel, c.pois, metric));
  OATPP_LOGI(
      "Optimizer", "Job finished in %lld ms",
      (long long)duration_cast<milliseconds>(high_resolution_clock::now() - t1)
          .count());
  return res;
}
} // namespace atr
