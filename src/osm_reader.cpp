...
struct WayHandler : public osmium::handler::Handler {
  ...
  void way(const osmium::Way &way) {
    ...
    if (validTypes.find(highway) == validTypes.end())
      return;

    float speed = parseSpeed(way.get_value_by_key("maxspeed"), highway);
    if (speed < 1.0f)
      speed = 1.0f; // Prevent division by zero
    ...
    for (size_t i = 0; i + 1 < nodes.size(); ++i) {
      try {
        ...
        if (!oneWay)
          m_buildEdges.push_back({v, u, static_cast<float>(dist), duration});
  ...
  float parseSpeed(const char *maxspeed, const char *highway) {
    if (maxspeed) {
      try {
        return std::stof(maxspeed);
      } catch (...) {
      }
    }
    std::string h(highway);
    if (h.find("motorway") != std::string::npos)
      return 130.0f;
    if (h.find("trunk") != std::string::npos)
      return 100.0f;
    if (h.find("primary") != std::string::npos)
      return 100.0f;
    if (h.find("secondary") != std::string::npos)
      return 80.0f;
  ...
  bool isOneWay(const osmium::Way &way) {
    ...
    if (ow && std::string(ow) == "yes")
      return true;
    const char *highway = way.get_value_by_key("highway");
    if (highway && std::string(highway) == "motorway")
      return true;
    return false;
  }
...
OSMReader::readPbf(const std::filesystem::path &path, OSMConfig config) {
  ...
  osmium::handler::NodeLocationsForWays<WayHandler::LocationIndex>
      locationHandler{index};
  osmium::apply(reader, locationHandler, handler);
  reader.close();
  return std::make_unique<StaticGraph>(std::move(handler.m_internalNodes),
                                       handler.m_buildEdges);
}
